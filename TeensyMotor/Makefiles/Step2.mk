...
# ~
		...
		$(call SHOW,"7.12-LINK",$@,.)
		$(call TRACE,"7-LINK",$@,.)
		$(CXX) $(LDFLAGS) $(OUT_PREPOSITION)$@ -Wl,--start-group $(OBJDIR)/syscalls.c.o $(LOCAL_OBJS) $(OBJDIR)/variant.cpp.o $(INCLUDE_A) $(TARGET_A) -Wl,--end-group
# ~~
...
ifneq ($(SOFTDEVICE),)
	$(call SHOW,"8.2-COPY",$@,$<)
	$(call TRACE,"8-COPY",$@,$<)
	$(MERGE_PATH)/$(MERGE_EXEC) $(SOFTDEVICE_HEX) -intel $(CURRENT_DIR)/$@ -intel $(OUT_PREPOSITION)$(CURRENT_DIR)/combined.hex $(MERGE_OPTS)
	mv $(CURRENT_DIR)/combined.hex $(CURRENT_DIR)/$@
endif
# ~~
...
# ~~
  ...
  ifeq ($(UPLOADER_PROTOCOL),tilib)
    ...
    ifneq ($(MAKECMDGOALS),debug)
		...
		$(call TRACE,"10-UPLOAD",$(UPLOADER))
#		-killall openocd
		$(UPLOADER_EXEC) $(UPLOADER_OPTS) -f $(TARGET_ELF)
		@if [ -d ./dll ]; then rm -R ./dll; fi
    endif
...
# ~
    ...
    ifeq ($(SPARK_NAME),)
		...
		@if [ -z '$(SPARK_NAME)' ] ; then echo 'ERROR No core found' ; exit 1 ; fi
